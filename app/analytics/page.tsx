  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        ...
        <div className="bg-primary text-white p-3 md:p-6 rounded-2xl shadow-lg relative overflow-hidden">
          <p className="text-xs md:text-sm opacity-80 mb-1">Total Followers</p>
          <h3 className="text-2xl md:text-4xl font-bold mb-2 md:mb-4">6,302</h3>
          <p className="text-[10px] md:text-xs italic opacity-60">As of February 11, 2026</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 min-w-0">
          <h3 className="text-base md:text-lg font-semibold text-gray-800 mb-4 md:mb-6">Engagement Rate Trend</h3>
          <div className="h-48 md:h-64 w-full min-w-0 min-h-48">
            {chartsReady && (
        ...
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 min-w-0">
          <div className="flex flex-wrap justify-between items-center mb-4 md:mb-6 gap-2">
            <h3 className="text-base md:text-lg font-semibold text-gray-800">Follower Growth (14 Days)</h3>
            <div className="flex gap-4 text-xs">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-secondary"></span> Total</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-primary"></span> New</span>
            </div>
          </div>
          <div className="h-48 md:h-64 w-full min-w-0 min-h-48">
            {chartsReady && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 min-w-0">
          <h3 className="text-base md:text-lg font-semibold text-gray-800 mb-4 md:mb-6">Reach and Impressions</h3>
          <div className="h-48 md:h-64 w-full min-w-0 min-h-48">
            {chartsReady && (
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
        ...
        <div className="lg:col-span-1 bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center min-w-0">
          <h3 className="text-base md:text-lg font-semibold text-gray-800 w-full mb-2">Platform Comparison</h3>
          <div className="h-48 md:h-64 w-full relative min-w-0 min-h-48">
            {chartsReady && (
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <PieChart>
                  <Pie
...
function StatCard({ title, value, date }: { title: string, value: string, date: string }) {
  return (
    <div className="bg-white p-3 md:p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center">
      <p className="text-xs md:text-sm text-gray-500 font-medium mb-1">{title}</p>
      <h3 className="text-2xl md:text-4xl font-bold text-gray-900 mb-2 md:mb-4">{value}</h3>
      <p className="text-[10px] md:text-xs italic text-gray-400">As of {date}</p>
    </div>
  );
}